...
//
#ifndef USDPHYSX_GENERATED_SURFACEVELOCITYAPI_H
#define USDPHYSX_GENERATED_SURFACEVELOCITYAPI_H

/// \file usdPhysX/surfaceVelocityAPI.h
...
/// \class UsdPhysXSurfaceVelocityAPI
///
/// 
/// PhysxSurfaceVelocityAPI enables surface velocity simulation that injects velocity to the solver through internal contact modify callback.
/// 
/// Surface velocity can be used typically for conveyer belt simulation. It must be applied to a prim with UsdPhysicsRigidBodyAPI.
/// 
///
class UsdPhysXSurfaceVelocityAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdPhysXSurfaceVelocityAPI on UsdPrim \p prim .
    /// Equivalent to UsdPhysXSurfaceVelocityAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdPhysXSurfaceVelocityAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdPhysXSurfaceVelocityAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdPhysXSurfaceVelocityAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdPhysXSurfaceVelocityAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Destructor.
    USDPHYSX_API
    virtual ~UsdPhysXSurfaceVelocityAPI();
    ...
    /// Return a UsdPhysXSurfaceVelocityAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdPhysXSurfaceVelocityAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDPHYSX_API
    static UsdPhysXSurfaceVelocityAPI
    ...
    /// This information is stored by adding "PhysxSchemaPhysxSurfaceVelocityAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdPhysXSurfaceVelocityAPI object is returned upon success. 
    /// An invalid (or empty) UsdPhysXSurfaceVelocityAPI object is returned upon 
