...
//
#ifndef USDPHYSX_GENERATED_ISOSURFACEAPI_H
#define USDPHYSX_GENERATED_ISOSURFACEAPI_H

/// \file usdPhysX/isosurfaceAPI.h
...
public:
    ...
    /// Construct a UsdPhysXIsosurfaceAPI on UsdPrim \p prim .
    /// Equivalent to UsdPhysXIsosurfaceAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdPhysXIsosurfaceAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdPhysXIsosurfaceAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdPhysXIsosurfaceAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdPhysXIsosurfaceAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdPhysXIsosurfaceAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdPhysXIsosurfaceAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDPHYSX_API
    static UsdPhysXIsosurfaceAPI
    ...
    /// 
    /// \return A valid UsdPhysXIsosurfaceAPI object is returned upon success. 
    /// An invalid (or empty) UsdPhysXIsosurfaceAPI object is returned upon 
