...
//
#ifndef USDPHYSX_GENERATED_VEHICLETIREAPI_H
#define USDPHYSX_GENERATED_VEHICLETIREAPI_H

/// \file usdPhysX/vehicleTireAPI.h
...
/// \class UsdPhysXVehicleTireAPI
///
/// 
/// Properties of a PhysX vehicle tire.
/// 
/// If the tire setup does not need to be shared among vehicle instances, it can be applied to the prim which has PhysxVehicleWheelAttachmentAPI applied. 
/// If the intent is to share the tire setup, PhysxVehicleTireAPI can be applied to a separate prim which can be linked to (see PhysxVehicleWheelAttachmentAPI).
/// 
///
class UsdPhysXVehicleTireAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdPhysXVehicleTireAPI on UsdPrim \p prim .
    /// Equivalent to UsdPhysXVehicleTireAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdPhysXVehicleTireAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdPhysXVehicleTireAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdPhysXVehicleTireAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdPhysXVehicleTireAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Destructor.
    USDPHYSX_API
    virtual ~UsdPhysXVehicleTireAPI();
    ...
    /// Return a UsdPhysXVehicleTireAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdPhysXVehicleTireAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDPHYSX_API
    static UsdPhysXVehicleTireAPI
    ...
    /// This information is stored by adding "PhysxSchemaPhysxVehicleTireAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdPhysXVehicleTireAPI object is returned upon success. 
    /// An invalid (or empty) UsdPhysXVehicleTireAPI object is returned upon 
...
public:
    ...
    /// 
    /// The friction value extracted from this graph will be multiplied with the friction value of the combination of ground material and tire type. 
    /// The array is expected to have 3 entries/points. For the first point, the first value is always zero and the second value is the friction available at zero longitudinal slip. 
    /// For the second point, the first value is the longitudinal slip with maximum friction and the second value is the maximum friction. 
    /// For the third point, the first value is the end point of the graph and the second value is the friction for slips greater than the end point of the graph. 
    /// Note that the friction value of the first point is typically smaller than the friction value of the second point and the friction value of the second point is typically larger than the friction value of the third point.
    /// 
...
public:
    ...
    /// 
    /// The first value of the parameter describes the minimum normalized load (load/restLoad) that gives a flat lateral stiffness response to load. 
    /// This value has to be greater or equal 0. The second value of the parameter defines the lateral stiffness when the response is flat (the max lateral stiffness, units: mass * distance / seconds / seconds). This value has to be positive.
    /// 
...
public:
    ...
    /// 
    /// Note: it is not possible for a vehicle to have some rest load values being user-defined and some being computed automatically. 
