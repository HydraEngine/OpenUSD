...
//
#ifndef USDPHYSX_GENERATED_RIGIDBODYAPI_H
#define USDPHYSX_GENERATED_RIGIDBODYAPI_H

/// \file usdPhysX/rigidBodyAPI.h
...
/// \class UsdPhysXRigidBodyAPI
///
/// 
/// PhysX rigid body extended parameters.
/// 
///
class UsdPhysXRigidBodyAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdPhysXRigidBodyAPI on UsdPrim \p prim .
    /// Equivalent to UsdPhysXRigidBodyAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdPhysXRigidBodyAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdPhysXRigidBodyAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdPhysXRigidBodyAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdPhysXRigidBodyAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdPhysXRigidBodyAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdPhysXRigidBodyAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDPHYSX_API
    static UsdPhysXRigidBodyAPI
    ...
    /// This information is stored by adding "PhysxSchemaPhysxRigidBodyAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdPhysXRigidBodyAPI object is returned upon success. 
    /// An invalid (or empty) UsdPhysXRigidBodyAPI object is returned upon 
