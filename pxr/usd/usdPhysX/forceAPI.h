...
//
#ifndef USDPHYSX_GENERATED_FORCEAPI_H
#define USDPHYSX_GENERATED_FORCEAPI_H

/// \file usdPhysX/forceAPI.h
...
/// as the value.
///
class UsdPhysXForceAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdPhysXForceAPI on UsdPrim \p prim .
    /// Equivalent to UsdPhysXForceAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdPhysXForceAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdPhysXForceAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdPhysXForceAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdPhysXForceAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdPhysXForceAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdPhysXForceAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDPHYSX_API
    static UsdPhysXForceAPI
    ...
    /// 
    /// \return A valid UsdPhysXForceAPI object is returned upon success. 
    /// An invalid (or empty) UsdPhysXForceAPI object is returned upon 
