...
HdSceneIndexAdapterSceneDelegate::HdSceneIndexAdapterSceneDelegate(HdSceneIndexBaseRefPtr inputSceneIndex,
    ...
    : HdSceneDelegate(parentIndex, delegateID),
      ...
      _cachedDirtyBits(0),
      _cachedPrimType() {
    std::string registeredName = ArchStringPrintf("delegate adapter: %s @ %s", delegateID.GetString().c_str(),
...
void HdSceneIndexAdapterSceneDelegate::_PrimAdded(const SdfPath &primPath, const TfToken &primType) {
    ...
    if (it != _primCache.end()) {
        ...
        std::atomic_store(&(entry.primvarDescriptors), std::shared_ptr<_PrimCacheEntry::PrimvarDescriptorsArray>());
    ...
    if (isResync) {
        ...
        if (GetRenderIndex().IsRprimTypeSupported(primType)) {
            HdDirtyBits allDirtyRprim = HdDirtyBitsTranslator::RprimLocatorSetToDirtyBits(primType, allDirty);
            GetRenderIndex().GetChangeTracker()._MarkRprimDirty(indexPath, allDirtyRprim);
        } else if (GetRenderIndex().IsSprimTypeSupported(primType)) {
            HdDirtyBits allDirtySprim = HdDirtyBitsTranslator::SprimLocatorSetToDirtyBits(primType, allDirty);
            GetRenderIndex().GetChangeTracker()._MarkSprimDirty(indexPath, allDirtySprim);
        } else if (GetRenderIndex().IsBprimTypeSupported(primType)) {
            HdDirtyBits allDirtyBprim = HdDirtyBitsTranslator::BprimLocatorSetToDirtyBits(primType, allDirty);
            GetRenderIndex().GetChangeTracker()._MarkBprimDirty(indexPath, allDirtyBprim);
        } else if (primType == HdPrimTypeTokens->instancer) {
            HdDirtyBits allDirtyInst = HdDirtyBitsTranslator::InstancerLocatorSetToDirtyBits(primType, allDirty);
...
void HdSceneIndexAdapterSceneDelegate::PrimsAdded(const HdSceneIndexBase &sender, const AddedPrimEntries &entries) {
    TRACE_FUNCTION();
...
void HdSceneIndexAdapterSceneDelegate::PrimsRemoved(const HdSceneIndexBase &sender, const RemovedPrimEntries &entries) {
    TRACE_FUNCTION();
...
void HdSceneIndexAdapterSceneDelegate::PrimsDirtied(const HdSceneIndexBase &sender, const DirtiedPrimEntries &entries) {
    TRACE_FUNCTION();
    ...
    for (const DirtiedPrimEntry &entry : entries) {
        ...
        if (GetRenderIndex().IsRprimTypeSupported(primType)) {
            ...
            if (entry.dirtyLocators == _cachedLocatorSet && primType == _cachedPrimType) {
                ...
                dirtyBits = HdDirtyBitsTranslator::RprimLocatorSetToDirtyBits(primType, entry.dirtyLocators);
            ...
            HdDirtyBits dirtyBits = HdDirtyBitsTranslator::SprimLocatorSetToDirtyBits(primType, entry.dirtyLocators);
            if (dirtyBits != HdChangeTracker::Clean) {
                GetRenderIndex().GetChangeTracker()._MarkSprimDirty(indexPath, dirtyBits);
            }
        } else if (GetRenderIndex().IsBprimTypeSupported(primType)) {
            HdDirtyBits dirtyBits = HdDirtyBitsTranslator::BprimLocatorSetToDirtyBits(primType, entry.dirtyLocators);
            if (dirtyBits != HdChangeTracker::Clean) {
                GetRenderIndex().GetChangeTracker()._MarkBprimDirty(indexPath, dirtyBits);
            }
        } else if (primType == HdPrimTypeTokens->instancer) {
            HdDirtyBits dirtyBits =
                    HdDirtyBitsTranslator::InstancerLocatorSetToDirtyBits(primType, entry.dirtyLocators);
            if (dirtyBits != HdChangeTracker::Clean) {
...
void HdSceneIndexAdapterSceneDelegate::PrimsRenamed(const HdSceneIndexBase &sender, const RenamedPrimEntries &entries) {
...
static void _GatherGeomSubsets(const SdfPath &parentPath,
    ...
    for (const SdfPath &childPath : sceneIndex->GetChildPrimPaths(parentPath)) {
        ...
        subsets.push_back({
                ...
                HdGeomSubset::Type::TypeFaceSet, childPath, materialId, indices});
    }
...
HdMeshTopology HdSceneIndexAdapterSceneDelegate::GetMeshTopology(SdfPath const &id) {
    ...
    HdIntArrayDataSourceHandle faceVertexIndicesDataSource = meshTopologySchema.GetFaceVertexIndices();
    ...
    HdMeshTopology meshTopology(scheme, orientation, faceVertexCountsDataSource->GetTypedValue(0.0f),
                                faceVertexIndicesDataSource->GetTypedValue(0.0f), holeIndices);
...
HdBasisCurvesTopology HdSceneIndexAdapterSceneDelegate::GetBasisCurvesTopology(SdfPath const &id) {
    ...
    HdBasisCurvesTopology result(type, basis, wrap, curveVertexCountsDataSource->GetTypedValue(0.0f), curveIndices);
...
HdVolumeFieldDescriptorVector HdSceneIndexAdapterSceneDelegate::GetVolumeFieldDescriptors(SdfPath const &volumeId) {
    ...
    HdVolumeFieldBindingSchema bindingSchema = HdVolumeFieldBindingSchema::GetFromParent(prim.dataSource);
...
SdfPath HdSceneIndexAdapterSceneDelegate::GetMaterialId(SdfPath const &id) {
    ...
    HdMaterialBindingsSchema materialBindings = HdMaterialBindingsSchema::GetFromParent(prim.dataSource);
...
static std::map<TfToken, VtValue> _GetHdParamsFromDataSource(HdMaterialNodeParameterContainerSchema containerSchema) {
    ...
    for (const auto &pName : pNames) {
        ...
        if (HdTokenDataSourceHandle colorSpaceDS = paramSchema.GetColorSpace()) {
            const TfToken cspName(SdfPath::JoinIdentifier(HdMaterialNodeParameterSchemaTokens->colorSpace, pName));
...
static void _Walk(const SdfPath &nodePath,
    ...
    if (HdMaterialConnectionVectorContainerSchema vectorContainerSchema = nodeSchema.GetInputConnections()) {
...
VtValue HdSceneIndexAdapterSceneDelegate::GetMaterialResource(SdfPath const &id) {
    ...
    const TfTokenVector renderContexts = GetRenderIndex().GetRenderDelegate()->GetMaterialRenderContexts();
...
VtValue HdSceneIndexAdapterSceneDelegate::GetCameraParamValue(SdfPath const &cameraId, TfToken const &paramName) {
    TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();
    ...
    if (!locator.IsEmpty()) {
        if (HdSampledDataSourceHandle const ds = HdSampledDataSource::Cast(
                    HdContainerDataSource::Get(cameraSchema.GetNamespacedProperties().GetContainer(), locator))) {
            return ds->GetValue(0.0f);
        }

        if (HdSampledDataSourceHandle const ds =
                    HdSampledDataSource::Cast(HdContainerDataSource::Get(cameraSchema.GetContainer(), locator))) {
            return ds->GetValue(0.0f);
        }
    ...
    HdSampledDataSourceHandle valueDs = HdSampledDataSource::Cast(cameraSchema.GetContainer()->Get(cameraSchemaToken));
    if (!valueDs) {
        return VtValue();
    }
    ...
    if (paramName == HdCameraSchemaTokens->projection) {
        ...
        return VtValue(proj == HdCameraSchemaTokens->perspective ? HdCamera::Perspective : HdCamera::Orthographic);
...
VtValue HdSceneIndexAdapterSceneDelegate::GetLightParamValue(SdfPath const &id, TfToken const &paramName) {
    TRACE_FUNCTION();
    ...
    HdContainerDataSourceHandle light = HdContainerDataSource::Cast(prim.dataSource->Get(HdLightSchemaTokens->light));
    if (light) {
...
GfRange2f _ToRange2f(GfVec4f const &v) { return GfRange2f(GfVec2f(v[0], v[1]), GfVec2f(v[2], v[3])); }
...
VtValue _GetRenderSettings(HdSceneIndexPrim prim, TfToken const &key) {
    HdContainerDataSourceHandle renderSettingsDs =
            HdContainerDataSource::Cast(prim.dataSource->Get(HdRenderSettingsSchemaTokens->renderSettings));
...
VtValue _GetRenderTerminalResource(HdSceneIndexPrim prim) {
    TRACE_FUNCTION();
...
VtValue HdSceneIndexAdapterSceneDelegate::_GetImageShaderValue(HdSceneIndexPrim prim, const TfToken &key) {
...
HdPrimvarDescriptorVector HdSceneIndexAdapterSceneDelegate::GetPrimvarDescriptors(SdfPath const &id,
    ...
    std::shared_ptr<_PrimCacheEntry::PrimvarDescriptorsArray> desired = _ComputePrimvarDescriptors(prim.dataSource);
...
HdSceneIndexAdapterSceneDelegate::_ComputePrimvarDescriptors(const HdContainerDataSourceHandle &primDataSource) {
...
HdExtComputationPrimvarDescriptorVector HdSceneIndexAdapterSceneDelegate::GetExtComputationPrimvarDescriptors(
    ...
    // Since multiple threads may arrive here at once, we only let the first one
    // write, and later threads discard their version.
    if (std::atomic_compare_exchange_strong((&it->second.extCmpPrimvarDescriptors), &expected, desired)) {
        return (*desired)[interpolation];
    } else {
        return (*expected)[interpolation];
    }
...
HdSceneIndexAdapterSceneDelegate::_ComputeExtCmpPrimvarDescriptors(const HdContainerDataSourceHandle &primDataSource) {
    if (!TF_VERIFY(primDataSource)) {
        return nullptr;
    }
    ...
    if (HdExtComputationPrimvarsSchema primvars = HdExtComputationPrimvarsSchema::GetFromParent(primDataSource)) {
        for (const TfToken &name : primvars.GetExtComputationPrimvarNames()) {
            HdExtComputationPrimvarSchema primvar = primvars.GetExtComputationPrimvar(name);
            if (!primvar) {
                continue;
            }
            ...
            if (HdTokenDataSourceHandle sourceComputationOutputDs = primvar.GetSourceComputationOutputName()) {
            ...
            descriptors[interpolation].push_back(
                    {name, interpolation, roleToken, sourceComputation, sourceComputationOutputName, valueType});
        }
    }
...
VtValue HdSceneIndexAdapterSceneDelegate::Get(SdfPath const &id, TfToken const &key) {
    ...
    if (prim.primType == HdPrimTypeTokens->drawTarget) {
        if (HdContainerDataSourceHandle drawTarget =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdPrimTypeTokens->drawTarget))) {
    ...
    if (HdLegacyPrimTypeIsVolumeField(prim.primType)) {
        HdContainerDataSourceHandle volumeField =
                HdContainerDataSource::Cast(prim.dataSource->Get(HdVolumeFieldSchemaTokens->volumeField));
        if (!volumeField) {
            return VtValue();
        }
    ...
    if (prim.primType == HdPrimTypeTokens->renderBuffer) {
        if (HdContainerDataSourceHandle renderBuffer =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdRenderBufferSchemaTokens->renderBuffer))) {
    ...
    if (prim.primType == HdPrimTypeTokens->cube) {
        if (HdContainerDataSourceHandle cubeSrc =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdCubeSchemaTokens->cube))) {
    ...
    if (prim.primType == HdPrimTypeTokens->sphere) {
        if (HdContainerDataSourceHandle sphereSrc =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdSphereSchemaTokens->sphere))) {
    ...
    if (prim.primType == HdPrimTypeTokens->cylinder) {
        if (HdContainerDataSourceHandle cylinderSrc =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdCylinderSchemaTokens->cylinder))) {
    ...
    if (prim.primType == HdPrimTypeTokens->cone) {
        if (HdContainerDataSourceHandle coneSrc =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdConeSchemaTokens->cone))) {
    ...
    if (prim.primType == HdPrimTypeTokens->capsule) {
        if (HdContainerDataSourceHandle capsuleSrc =
                    HdContainerDataSource::Cast(prim.dataSource->Get(HdCapsuleSchemaTokens->capsule))) {
    ...
    if (prim.primType == HdPrimTypeTokens->coordSys) {
        ...
        if (key == nameKey) {
            if (HdTokenDataSourceHandle const nameDs = HdCoordSysSchema::GetFromParent(prim.dataSource).GetName()) {
    ...
    // delegate.
    if (HdTypedSampledDataSource<HdSceneDelegate *>::Handle sdDs = HdTypedSampledDataSource<HdSceneDelegate *>::Cast(
...
VtValue HdSceneIndexAdapterSceneDelegate::_GetPrimvar(SdfPath const &id, TfToken const &key, VtIntArray *outIndices) {
    TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();
    if (outIndices) {
        outIndices->clear();
    ...
    return _GetPrimvar(HdPrimvarsSchema::GetFromParent(prim.dataSource).GetContainer(), key, outIndices);

    return VtValue();
}

VtValue HdSceneIndexAdapterSceneDelegate::_GetPrimvar(const HdContainerDataSourceHandle &primvarsDataSource,
                                                      TfToken const &key,
                                                      VtIntArray *outIndices) {
...
size_t HdSceneIndexAdapterSceneDelegate::SamplePrimvar(
        SdfPath const &id, TfToken const &key, size_t maxSampleCount, float *sampleTimes, VtValue *sampleValues) {
    return _SamplePrimvar(id, key, _fallbackStartTime, _fallbackEndTime, maxSampleCount, sampleTimes, sampleValues,
                          nullptr);
}
...
size_t HdSceneIndexAdapterSceneDelegate::SamplePrimvar(SdfPath const &id,
    ...
    return _SamplePrimvar(id, key, startTime, endTime, maxSampleCount, sampleTimes, sampleValues, nullptr);
}
...
size_t HdSceneIndexAdapterSceneDelegate::SampleIndexedPrimvar(SdfPath const &id,
                                                              ...
                                                              VtValue *sampleValues,
                                                              VtIntArray *sampleIndices) {
    return _SamplePrimvar(id, key, startTime, endTime, maxSampleCount, sampleTimes, sampleValues, sampleIndices);
}
...
size_t HdSceneIndexAdapterSceneDelegate::_SamplePrimvar(SdfPath const &id,
    ...
    if (prim.dataSource->Get(HdSceneIndexEmulationTokens->sceneDelegate)) {
        ...
        const bool isVarying = valueSource->GetContributingSampleTimesForInterval(startTime, endTime, &times);
        if (isVarying) {
            if (times.empty()) {
                TF_CODING_ERROR(
...
GfMatrix4d HdSceneIndexAdapterSceneDelegate::GetInstancerTransform(SdfPath const &id) { return GetTransform(id); }

size_t HdSceneIndexAdapterSceneDelegate::SampleTransform(SdfPath const &id,
                                                         size_t maxSampleCount,
                                                         float *sampleTimes,
                                                         GfMatrix4d *sampleValues) {
    return SampleTransform(id, _fallbackStartTime, _fallbackEndTime, maxSampleCount, sampleTimes, sampleValues);
}
...
std::vector<VtArray<TfToken>> HdSceneIndexAdapterSceneDelegate::GetInstanceCategories(SdfPath const &instancerId) {
    ...
    if (HdInstanceCategoriesSchema instanceCategories = HdInstanceCategoriesSchema::GetFromParent(prim.dataSource)) {
...
VtIntArray HdSceneIndexAdapterSceneDelegate::GetInstanceIndices(SdfPath const &instancerId,
    ...
    if (HdInstancerTopologySchema instancerTopology = HdInstancerTopologySchema::GetFromParent(prim.dataSource)) {
...
TfTokenVector HdSceneIndexAdapterSceneDelegate::GetExtComputationSceneInputNames(SdfPath const &computationId) {
    ...
    const HdExtComputationSchema extComputation = HdExtComputationSchema::GetFromParent(prim.dataSource);
    const HdSampledDataSourceContainerSchema inputValues = extComputation.GetInputValues();
    return inputValues.GetNames();
}

VtValue HdSceneIndexAdapterSceneDelegate::GetExtComputationInput(SdfPath const &computationId, TfToken const &input) {
    TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();
    ...
    if (input == HdTokens->dispatchCount) {
        HdSizetDataSourceHandle const dispatchDs = extComputation.GetDispatchCount();
        if (!dispatchDs) {
            return {};
        }
    ...
    if (input == HdTokens->elementCount) {
        HdSizetDataSourceHandle const elementDs = extComputation.GetElementCount();
        if (!elementDs) {
            return {};
        }
...
size_t HdSceneIndexAdapterSceneDelegate::SampleExtComputationInput(SdfPath const &computationId,
                                                                   ...
                                                                   float *sampleTimes,
                                                                   VtValue *sampleValues) {
    return SampleExtComputationInput(computationId, input, _fallbackStartTime, _fallbackEndTime, maxSampleCount,
...
HdExtComputationInputDescriptorVector HdSceneIndexAdapterSceneDelegate::GetExtComputationInputDescriptors(
    ...
    const HdSceneIndexPrim prim = _GetInputPrim(computationId);
    const HdExtComputationSchema extComputation = HdExtComputationSchema::GetFromParent(prim.dataSource);
    const HdExtComputationInputComputationContainerSchema inputComputations = extComputation.GetInputComputations();
    const TfTokenVector names = inputComputations.GetNames();
    result.reserve(names.size());
    for (const TfToken &name : names) {
        const HdExtComputationInputComputationSchema input = inputComputations.Get(name);
        if (!input) {
            continue;
        }
        HdExtComputationInputDescriptor desc;
        desc.name = name;
        if (HdPathDataSourceHandle const srcDs = input.GetSourceComputation()) {
            desc.sourceComputationId = srcDs->GetTypedValue(0);
        }
        if (HdTokenDataSourceHandle const srcNameDs = input.GetSourceComputationOutputName()) {
...
HdExtComputationOutputDescriptorVector HdSceneIndexAdapterSceneDelegate::GetExtComputationOutputDescriptors(
        SdfPath const &computationId) {
    TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();
    ...
    const HdSceneIndexPrim prim = _GetInputPrim(computationId);
    const HdExtComputationSchema extComputation = HdExtComputationSchema::GetFromParent(prim.dataSource);
    const HdExtComputationOutputContainerSchema outputs = extComputation.GetOutputs();
...
std::string HdSceneIndexAdapterSceneDelegate::GetExtComputationKernel(SdfPath const &computationId) {
    ...
    if (HdExtComputationSchema extComputation = HdExtComputationSchema::GetFromParent(prim.dataSource)) {
...
void HdSceneIndexAdapterSceneDelegate::InvokeExtComputation(SdfPath const &computationId,
                                                            HdExtComputationContext *const context) {
    TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();

    const HdSceneIndexPrim prim = _GetInputPrim(computationId);
    const HdExtComputationSchema extComputation = HdExtComputationSchema::GetFromParent(prim.dataSource);
    HdExtComputationCpuCallbackDataSourceHandle const ds = extComputation.GetCpuCallback();
    if (!ds) {
        return;
    }
    HdExtComputationCpuCallbackSharedPtr const callback = ds->GetTypedValue(0.0f);
    if (!callback) {
        return;
    }
...
void HdSceneIndexAdapterSceneDelegate::Sync(HdSyncRequestVector *request) {
    ...
    if (!_sceneDelegatesBuilt) {
        ...
        _primCache.ParallelForEach([this, &sds](const SdfPath &k, const _PrimCacheEntry &v) {
            ...
            HdTypedSampledDataSource<HdSceneDelegate *>::Handle ds = HdTypedSampledDataSource<HdSceneDelegate *>::Cast(
                    prim.dataSource->Get(HdSceneIndexEmulationTokens->sceneDelegate));
            if (!ds) {
                return;
            }
...
HdDisplayStyle HdSceneIndexAdapterSceneDelegate::GetDisplayStyle(SdfPath const &id) {
    ...
    HdSceneIndexPrim prim = _GetInputPrim(id);
    if (HdLegacyDisplayStyleSchema styleSchema = HdLegacyDisplayStyleSchema::GetFromParent(prim.dataSource)) {
