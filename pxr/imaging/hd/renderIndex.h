...
public:
    ...
    [[nodiscard]] HD_API SdfPath GetRprimPathFromPrimId(int primId) const;
    ...
    HD_API
    HdDrawItemPtrVector GetDrawItems(const SdfPath& rprimId, HdReprSelector const& colReprSelector);
    ...
    [[nodiscard]] HdChangeTracker const& GetChangeTracker() const { return _tracker; }
    ...
    [[nodiscard]] HD_API bool IsRprimTypeSupported(TfToken const& typeId) const;
    ...
    bool HasRprim(SdfPath const& id) { return _rprimMap.find(id) != _rprimMap.end(); }

    /// Returns the rprim of id
    [[nodiscard]] HD_API HdRprim const* GetRprim(SdfPath const& id) const;

    /// Returns the scene delegate for the given rprim
    [[nodiscard]] HD_API HdSceneDelegate* GetSceneDelegateForRprim(SdfPath const& id) const;

    /// Query function to return the id's of the scene delegate and instancer
    /// associated with the Rprim at the given path.
    HD_API
    bool GetSceneDelegateAndInstancerIds(SdfPath const& id, SdfPath* sceneDelegateId, SdfPath* instancerId) const;

    /// Returns the render tag for the given rprim
    [[nodiscard]] HD_API TfToken GetRenderTag(SdfPath const& id) const;
    ...
    bool HasInstancer(SdfPath const& id) { return _instancerMap.find(id) != _instancerMap.end(); }

    /// Returns the instancer of id
    [[nodiscard]] HD_API HdInstancer* GetInstancer(SdfPath const& id) const;
    ...
    bool HasTask(SdfPath const& id) { return _taskMap.find(id) != _taskMap.end(); }

    /// Returns the task for the given \p id.
    [[nodiscard]] HD_API HdTaskSharedPtr const& GetTask(SdfPath const& id) const;
    ...
    [[nodiscard]] HD_API bool IsSprimTypeSupported(TfToken const& typeId) const;
    ...
    [[nodiscard]] HD_API HdSprim* GetSprim(TfToken const& typeId, SdfPath const& id) const;
    ...
    [[nodiscard]] HD_API HdSprim* GetFallbackSprim(TfToken const& typeId) const;
    ...
    [[nodiscard]] HD_API bool IsBprimTypeSupported(TfToken const& typeId) const;
    ...
    [[nodiscard]] HD_API HdBprim* GetBprim(TfToken const& typeId, SdfPath const& id) const;
    ...
    [[nodiscard]] HD_API HdBprim* GetFallbackBprim(TfToken const& typeId) const;
    ...
    /// through emulation.
    [[nodiscard]] HD_API HdSceneIndexBaseRefPtr GetTerminalSceneIndex() const;
    ...
    [[nodiscard]] HD_API HdRenderDelegate* GetRenderDelegate() const;

    // The render delegate may require access to a render context / device
    // that is provided by the application.
    [[nodiscard]] HD_API HdDriverVector const& GetDrivers() const;

    /// Returns a shared ptr to the resource registry of the current render
    /// delegate.
    [[nodiscard]] HD_API HdResourceRegistrySharedPtr GetResourceRegistry() const;
    ...
    [[nodiscard]] HD_API std::string GetInstanceName() const;
...
private:
    ...
    // to be marked HD_API.
    HD_API
    void _TrackDelegateTask(HdSceneDelegate* delegate, SdfPath const& taskId, HdTaskCreateFnc taskCreateFnc);
